...
tf.app.flags.DEFINE_bool('full_annotation_flag', True, '')
            ...
            with tf.name_scope(config.clone_scopes[clone_idx]) as clone_scope:
                with tf.device(gpu) as clone_device:
                    ...
                    if FLAGS.model_version == 1:
                        net = UNetBlocksMS.UNet(b_image, b_mask_image, None, is_training=True,
                    ...
                    if FLAGS.update_center:
                        if FLAGS.update_center_strategy == 1:
                            ...
                            update_center_op = net.update_centers_V2()
                        else:
...
def train(train_op):
    ...
    real_train_dir = os.path.join(
        FLAGS.train_dir,
        '%s-%s-%s-%s-%s-%s-%s' % (FLAGS.dataset_name, FLAGS.decoder, str(FLAGS.update_center), str(FLAGS.update_center_strategy),
