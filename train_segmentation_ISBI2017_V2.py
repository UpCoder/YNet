...
# V2 代表的意思是我们的tfrecord不是原来的图像，而是原始的像素值(进行了阶段操作)
...
def create_dataset_batch_queue(dataset, preprocessing_flag=True):
    ...
    with tf.device('/cpu:0'):
        ...
        image = tf.expand_dims(image[:, :, 1], axis=2)
...
def create_clones(batch_queue):        
                ...
                with tf.device(gpu) as clone_device:
                    ...
                    if FLAGS.attention_flag:
                        ...
                        if FLAGS.center_block_flag:
                            ...
                            print('dense_connection_flag', FLAGS.dense_connection_flag)
                            print('learnable_connection_flag', FLAGS.learnable_connection_flag)
                            net = UNetBlocksMS.UNet(b_image, b_mask_image, b_liver_mask, is_training=True,
                                                    decoder=FLAGS.decoder,
                                                    update_center_flag=FLAGS.update_center, batch_size=FLAGS.batch_size,
                                                    update_center_strategy=FLAGS.update_center_strategy,
                                                    num_centers_k=FLAGS.num_centers_k,
                                                    full_annotation_flag=FLAGS.full_annotation_flag,
                                                    output_shape_tensor=random_scale,
                                                    pixel_cls_weight=b_pixel_cls_weight,
                                                    dense_connection_flag=FLAGS.dense_connection_flag,
                                                    learnable_merged_flag=FLAGS.learnable_connection_flag)
                        else:
                            print('use the UNet')
                    ...
                    if FLAGS.update_center and not FLAGS.full_annotation_flag:
    ...
    if FLAGS.update_center and FLAGS.update_center_strategy == 1 and not FLAGS.full_annotation_flag:
...
def train(train_op):
    ...
    if not FLAGS.full_annotation_flag:
        ...
        real_train_dir = FLAGS.train_dir + '-' + FLAGS.decoder + '-' + 'fully'
    if not FLAGS.update_center:
        real_train_dir = real_train_dir + '-wo_update'
    if FLAGS.attention_flag:
    ...
    real_train_dir = os.path.join(
        FLAGS.train_dir,
        '%s-%s-%s-%s-%s-%s-%s' % ('', FLAGS.decoder, str(FLAGS.update_center), str(FLAGS.update_center_strategy),
                                  str(FLAGS.attention_flag), str(FLAGS.dense_connection_flag),
                                  str(FLAGS.learnable_connection_flag))
    )
    print('real_train_dir is ', real_train_dir)
    # real_train_dir = FLAGS.dataset_name + '-' + real_train_dir
