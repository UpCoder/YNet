...
class TaskNetworkModuleV2(object):
    ...
    def __init__(self, input_tensors, output_dim, output_shape, arg_sc, name, decoder='upsampling', is_training=True,
                 hidden_dim=128, skip_connection=True, last_layer_activation=tf.nn.relu):
        last_output = None
...
class UNet(object):
    ...
    def _logits_to_scores(self):
        ...
        tf.summary.image('pred_mask',
                         ...
                         max_outputs=1)
        tf.summary.image('not_pred_mask',
                         tf.cast(tf.expand_dims(self.pixel_cls_scores[:, :, :, 0], axis=3) * 250.0, tf.uint8),
                         max_outputs=1)
        ...
        tf.summary.scalar('max_pixel_cls_scores', tf.reduce_max(self.pixel_cls_scores_flatten))
        tf.summary.scalar('min_pixel_cls_scores', tf.reduce_min(self.pixel_cls_scores_flatten))
        tf.summary.scalar('max_pixel_cls_scores_sample',
                          tf.reduce_max(tf.reduce_mean(self.pixel_cls_scores_flatten, axis=[1, 2])))
        tf.summary.scalar('min_pixel_cls_scores_sample',
                          tf.reduce_min(tf.reduce_mean(self.pixel_cls_scores_flatten, axis=[1, 2])))
        tf.summary.scalar('pixel_cls_score_variance', tf.reduce_mean(
            (self.pixel_cls_scores_flatten - tf.reduce_mean(self.pixel_cls_scores_flatten)) ** 2))
    ...
    def _get_weakly_label_loss(self):
        ...
        def _get_wealy_label_loss_sample(pixel_recovery_features_sample, centers_sample, pos_mask_sample, input_mask_sample):
            ...
            vis_tensor = tf.cast(tf.cast(tf.equal(assign_label, 0), tf.float32) * 200, tf.uint8)
        ...
        l2losses, vis_tensors = tf.map_fn(lambda
                             ...
                             (pixel_recovery_features, self.centers, self.pos_mask, self.inputs_mask),
    ...
    def _get_weakly_label_loss_with_center(self):
        ...
        def _update_single_sample(pixel_recovery_feature_sample, pos_mask_sample, input_mask_sample, liver_mask_sample, centers):
            '''
            针对每一个sample计算其类中心
