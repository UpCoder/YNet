...
def _process_image(dataset_dir, case_name, weakly_label_flag=False):
    ...
    """
    # Read the image file.
    filename = glob(os.path.join(dataset_dir, case_name, 'images', '*.png'))[0]
    print('filename is ', filename)
    image_data = tf.gfile.FastGFile(filename, 'rb').read()
    if weakly_label_flag is False:
        mask_path = os.path.join(dataset_dir, case_name, 'whole_mask.png')
    else:
        mask_path = os.path.join(dataset_dir, case_name, 'weakly_label_whole_mask.png')
...
def _add_to_tfrecord(dataset_dir, case_name, tfrecord_writer, weakly_label_flag):
    ...
    image_data, mask_data = _process_image(dataset_dir, case_name, weakly_label_flag)

    example = _convert_to_example(image_data, mask_data)
...
def run(dataset_dir, output_dir, shuffling=True, name='medicalimage', weakly_label_flag=False):
    ...
    while i < len(case_names):
        ...
        with tf.python_io.TFRecordWriter(tf_filename) as tfrecord_writer:
            ...
            while i < len(case_names) and j < SAMPLES_PER_FILES:
                ...
                _add_to_tfrecord(dataset_dir, case_name, tfrecord_writer, weakly_label_flag)
                i += 1
                j += 1
            fidx += 1
...
if __name__ == '__main__':
    dataset_dir = '/home/give/Documents/dataset/data-science-bowl-2018/stage1_train_BW'
    output_dir = '/home/give/Documents/dataset/data-science-bowl-2018/stage1_train_BW_tfrecords'
    weakly_label_flag = True
    # preprocessing_dir(dataset_dir)
    run(dataset_dir, output_dir, name='DSB2018', weakly_label_flag=weakly_label_flag)
